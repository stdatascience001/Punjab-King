...
const PAGE_TITLE = 'Flash Message';
const FIELD: 'message' | 'flashMessage' = 'flashMessage';
const COLUMN_LABEL = 'Flash Message';
...
export const FlashMessageManagePage: React.FC = () => {
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      console.warn('Failed to load flash messages:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdate = async (roleId: number) => {
    ...
    try {
      ...
      alert(err.message || 'Failed to update flash message');
    } finally {
