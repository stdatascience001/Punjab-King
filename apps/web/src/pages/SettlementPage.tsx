...
export const SettlementPage: React.FC<SettlementPageProps> = ({ onNavigate }) => {
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      const res = await apiRequest<SettlementRow[]>(`/vouchers/settlement-rows?fromDate=${fromDate}&toDate=${toDate}`);
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-200 font-sans text-xs whitespace-nowrap">
              {loading ? (
                ...
                <tr><td colSpan={8} className="py-14 text-center text-slate-400 font-medium">No records found for this month.</td></tr>
              ) : (
